    ...
    <View ref={rootRef} style={{ flex: 1, backgroundColor: COLOR.bg }}>
      ...
      />
        ...
        <RequestLeave
            ...
            onSubmitted={async () => {
                try {
                    const token = await AsyncStorage.getItem("accessToken"); // 登录时存进去的
                    const resp = await fetch(`${API_BASE_URL}/myroster/shift/99003/leave-request/submit`, {
                        method: "POST",
                        headers: {
                    ...
                    console.log("LEAVE SUBMIT", resp.status, text);
                    ...
                    alert(`Leave submit failed: ${e.message}`);
                }
            }}
